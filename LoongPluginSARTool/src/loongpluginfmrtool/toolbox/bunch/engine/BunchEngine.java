 ...
 * @version 1.0
 */
package loongpluginfmrtool.toolbox.bunch.engine;

import loongpluginfmrtool.toolbox.bunch.BunchPreferences;
import loongpluginfmrtool.toolbox.bunch.Cluster;
import loongpluginfmrtool.toolbox.bunch.ClusteringMethod;
import loongpluginfmrtool.toolbox.bunch.Configuration;
import loongpluginfmrtool.toolbox.bunch.DependencyFileParser;
import loongpluginfmrtool.toolbox.bunch.GAConfiguration;
...
import loongpluginfmrtool.toolbox.bunch.SATechnique;
import loongpluginfmrtool.toolbox.bunch.api.BunchAPI;
import loongpluginfmrtool.toolbox.bunch.api.BunchAsyncNotify;
import loongpluginfmrtool.toolbox.bunch.api.BunchProperties;
import loongpluginfmrtool.toolbox.bunch.api.ProgressCallbackInterface;
import loongpluginfmrtool.toolbox.bunch.stats.StatsManager;

import java.util.*;
import java.beans.*;

import loongpluginfmrtool.toolbox.bunch.*;
import loongpluginfmrtool.toolbox.bunch.api.*;
...
public class BunchEngine {
  ...
  StatsManager stats = loongpluginfmrtool.toolbox.bunch.stats.StatsManager.getInstance();
...

{
  ...
  boolean initClustering()
  {
    try{
      ...
      {
        loongpluginfmrtool.toolbox.bunch.api.BunchMDG mdgObj = (loongpluginfmrtool.toolbox.bunch.api.BunchMDG)
            bunchArgs.get(BunchProperties.MDG_GRAPH_OBJECT);

        initialGraph_d = loongpluginfmrtool.toolbox.bunch.util.BunchUtilities.toInternalGraph(mdgObj);
      ...
      cbInterfaceObj = (ProgressCallbackInterface)bunchArgs.get(loongpluginfmrtool.toolbox.bunch.api.BunchAPI.CALLBACK_OBJECT_REF);
      Integer iTmp = (Integer)bunchArgs.get(loongpluginfmrtool.toolbox.bunch.api.BunchAPI.CALLBACK_OBJECT_FREQ);
      if(iTmp != null)
  ...
  boolean runClusteringAsync(final BunchAsyncNotify nObject)
  {
    ...
    nObject.setStatus(loongpluginfmrtool.toolbox.bunch.api.BunchAsyncNotify.STATUS_RUNNING);
      ...
      //Runnable runThread = new Runnable()
    {
      ...
      {
        nObject.setStatus(loongpluginfmrtool.toolbox.bunch.api.BunchAsyncNotify.STATUS_DONE);
        nObject.notifyDone();
      }
    };
  ...
  boolean runMQCalc()
  {
    ...
    double mqResult = loongpluginfmrtool.toolbox.bunch.util.MQCalculator.CalcMQ(MQCalcMdgFileName,MQCalcSilFileName,MQCalcClass);
  ...
  boolean runPRCalc()
  {
    ...
    loongpluginfmrtool.toolbox.bunch.util.PrecisionRecallCalculator calc =
      new loongpluginfmrtool.toolbox.bunch.util.PrecisionRecallCalculator(expertF,clusterF);
...
class ExecuteClusteringEngineAsync
{
    ...
    public void run()
    {
      notifyObject.setStatus(loongpluginfmrtool.toolbox.bunch.api.BunchAsyncNotify.STATUS_RUNNING);
      ...
      //Runnable runThread = new Runnable()
      {
        ...
        public void finished()
        {
          notifyObject.setStatus(loongpluginfmrtool.toolbox.bunch.api.BunchAsyncNotify.STATUS_DONE);
