 ...
 *
 */
package loongpluginfmrtool.toolbox.bunch.BunchServer;

import loongpluginfmrtool.toolbox.bunch.Callback;
import loongpluginfmrtool.toolbox.bunch.Cluster;
import loongpluginfmrtool.toolbox.bunch.BunchServer.BSWindow;
import loongpluginfmrtool.toolbox.bunch.BunchServer.BunchSvrMsg;
import loongpluginfmrtool.toolbox.bunch.BunchServer.ClusterUsingVectorSAHC;
import loongpluginfmrtool.toolbox.bunch.BunchServer.DistribInit;
import loongpluginfmrtool.toolbox.bunch.BunchServer.IterationManager;
import loongpluginfmrtool.toolbox.bunch.BunchServer.ServerClusteringEngine;
import loongpluginfmrtool.toolbox.bunch.BunchServer.ServerClusteringProgress;
import loongpluginfmrtool.toolbox.bunch.BunchServer.ServerProperties;
...
public class BunchSvrMsgImpl extends PortableRemoteObject implements BunchSvrMsg{
  ...
  public boolean invokeMessage(String name, byte[] serializedClass)
  {
      if(name.equals("Init"))
      {
         DistribInit di = (DistribInit)loongpluginfmrtool.toolbox.bunch.util.BunchUtilities.fromByteArray(serializedClass);
      ...
      ******/
        ...
        loongpluginfmrtool.toolbox.bunch.SwingWorker worker_d = new loongpluginfmrtool.toolbox.bunch.SwingWorker(/*clusteringMethod_x*/) {
          public Object construct() {
            IterationManager starti = (IterationManager)loongpluginfmrtool.toolbox.bunch.util.BunchUtilities.fromByteArray(sc);
            Cluster workCluster = new Cluster(sp.theGraph,starti.clusterVector);
            ClusterUsingVectorSAHC cThread = new ClusterUsingVectorSAHC(parent,sp,workCluster,clientCallback);
            cThread.setProgressWindow(progressWindow);
            cThread.run();
