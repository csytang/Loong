...
//System.out.println("Moving node : " + currNode+" to cluster " + rndClustNameOrdering[j]);
                ...
                if ((loongpluginfmrtool.toolbox.bunch.util.BunchUtilities.compareGreater(c.getObjFnValue(),maxOF))||(acceptSAMove))
                {
...
//******************** THIS IS NEW EXPIREMENTAL CODE
    if (!loongpluginfmrtool.toolbox.bunch.util.BunchUtilities.compareGreater(maxOF,originalMax)) {
        ...
        for (int i=0; i<clusters.length; ++i) {
          ...
          for (; j<edges.length; ++j) {
            ...
            if ((!locks[currNode]) && (!locks[otherNode])) {
                ...
                if (loongpluginfmrtool.toolbox.bunch.util.BunchUtilities.compareGreater(c.getObjFnValue(),maxOF)) {
                    maxC.copyFromCluster(c);
