...
public class Stemmer {
	...
	public static void main(String[] args) {
		boolean outputStemmedFileToScreen = false;
		...
		for (int i = 0; i < args.length; i++)
			try {
				...
				File outputDir = new File(inputDir.getParent() + '/' + "ccs_stemmed/");
				...
				for (File f : filesList) {
					if (!f.isDirectory()) {
						...
						String outputFilename = outputDir.getAbsolutePath() + "/" + f.getName();
