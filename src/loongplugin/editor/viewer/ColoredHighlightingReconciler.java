...
import loongplugin.configuration.CodeSegmentCalculator;
...
//		fNOfRemovedPositions = fRemovedPositions.size();
	...
	private List<ColoredCLRProjectionAnnotation> oldAnnotations=new ArrayList<ColoredCLRProjectionAnnotation>();
...
//				+ addedPositions + " " + removedPositions);
		...
		CLREditor editor = fEditor;
		if (editor == null)
			return;

		
		updateCLRProjectionAnnotations(addedPositions, editor);
	...
	protected void updateCLRProjectionAnnotations(List<CodeSegment> addedPositions, CLREditor editor) {
		CLRProjectionAnnotationModel annotationModel = ((CLRJavaSourceViewer) editor
				.getViewer()).getCLRProjectionAnnotationModel();

		ColoredCLRProjectionAnnotation[] annotations = new ColoredCLRProjectionAnnotation[addedPositions
				.size()];
		...
		ArrayList<ColoredCLRProjectionAnnotation> knownPositions = new ArrayList<ColoredCLRProjectionAnnotation>(
				oldAnnotations);
		ArrayList<ColoredCLRProjectionAnnotation> savedPositions = new ArrayList<ColoredCLRProjectionAnnotation>();
		...
		for (Iterator<CodeSegment> i = addedPositions.iterator(); i.hasNext();) {
			...
			for (Iterator<ColoredCLRProjectionAnnotation> a = knownPositions
					.iterator(); a.hasNext();) {
				ColoredCLRProjectionAnnotation known = a.next();
		...
		for (int i = 0; i < addedPositions.size(); i++) {
			...
			ColoredCLRProjectionAnnotation annotation = new ColoredCLRProjectionAnnotation(false,addedPositions.get(i).getColors(),
					fEditor.getCLRAnnotatedFile().getProject(),pos);
...
//		fNOfRemovedPositions = 0;
	 ...
	 *            the highlightings
	 */
	public void install(CLREditor editor,
		...
		if (fEditor instanceof CLREditor) {
			((CLREditor) fEditor).addReconcileListener2(this);
		} else {
	...
	public void uninstall() {
		...
		if (fEditor != null) {
			if (fEditor instanceof CLREditor)
				...
				((CLREditor) fEditor).removeReconcileListener2(this);
			else
