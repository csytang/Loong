...
import loongplugin.configuration.CodeSegment;
...
public class ColoredHighlightingPresenter implements ITextPresentationListener,
	 ...
	 */
	private class CodeSegmentPositionUpdater implements IPositionUpdater {
		 ...
		 *            the new category.
		 */
		public CodeSegmentPositionUpdater(String category) {
			fCategory = category;
		}
		...
		public void update(DocumentEvent event) {
			...
			try {
				...
				for (int i = 0; i != positions.length; i++) {
					...
					CodeSegment position = (CodeSegment) positions[i];
		 ...
		 *            The event
		 */
		private void updateWithPrecedingEvent(CodeSegment position,
				DocumentEvent event) {
		 ...
		 *            The event
		 */
		private void updateWithSucceedingEvent(CodeSegment position,
				DocumentEvent event) {
		}
		 ...
		 *            The event
		 */
		private void updateWithIncludedEvent(CodeSegment position,
				DocumentEvent event) {
			int eventOffset = event.getOffset();
			String newText = event.getText();
			if (newText == null)
		 ...
		 *            The event
		 */
		private void updateWithOverEndEvent(CodeSegment position,
				DocumentEvent event) {
			String newText = event.getText();
			if (newText == null)
				newText = ""; //$NON-NLS-1$
		 ...
		 *            The event
		 */
		private void updateWithOverStartEvent(CodeSegment position,
				DocumentEvent event) {
			int eventOffset = event.getOffset();
		 ...
		 *            The event
		 */
		private void updateWithIncludingEvent(CodeSegment position,
				DocumentEvent event) {
			position.delete();
	...
	/** Position updater */
	private IPositionUpdater fPositionUpdater = new CodeSegmentPositionUpdater(
			getPositionCategory());
	 ...
	 * elements
	 */
	private List<CodeSegment> fPositions = new ArrayList<CodeSegment>();
	...
	public Runnable createUpdateRunnable(
		...
		final CodeSegment[] added = new CodeSegment[addedPositions.size()];
		addedPositions.toArray(added);
		final CodeSegment[] removed = new CodeSegment[removedPositions.size()];
	...
	public void updatePresentation(TextPresentation textPresentation,
			CodeSegment[] addedPositions, CodeSegment[] removedPositions) {
	 ...
	 */
	private void insertPosition(CodeSegment position) {
	...
	public void applyTextPresentation(TextPresentation textPresentation) {
		...
		if (n - i > 2) {
			...
			for (; i < n; i++) {
				CodeSegment position = fPositions.get(i);
				if (!position.isDeleted())
					ranges.add(createStyleRange(position));
			}
			...
			for (; i < n; i++) {
				...
				if (!position.isDeleted())
					textPresentation
							.replaceStyleRange(createStyleRange(position));
			}
		}
	}
	...
	public StyleRange createStyleRange(CodeSegment segment) {
		...
		if (!segment.getColors().isEmpty())
			background = FeatureModelManager.getCombinedColor(segment.getColors());
		...
		StyleRange styleRange = new StyleRange(segment.getOffset(), segment
				.getLength(), null, background/* , fontStyle */);
	...
	/**
	 * @return Returns <code>true</code> iff the current reconcile is
	 *         canceled.
	 *         <p>
	 ...
	 * @param highlighting
	 */
	private void addPositionFromUI(int offset, int length, CodeSegment segment) {
		synchronized (fPositionLock) {
			insertPosition(segment);
		}
		...
		String positionCategory = getPositionCategory();
		try {
			document.addPosition(positionCategory, segment);
		} catch (BadLocationException e) {
			// Should not happen
