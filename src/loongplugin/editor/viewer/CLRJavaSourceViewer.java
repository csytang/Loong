...
import loongplugin.editor.projection.CLRProjectionAnnotation;
...
import loongplugin.editor.projection.ICLRProjectionListener;


@SuppressWarnings("restriction")
public class CLRJavaSourceViewer extends JavaSourceViewer implements 
		ITextViewerExtension5 {
	
	public CLRJavaSourceViewer(Composite parent, IVerticalRuler ruler,
			IOverviewRuler overviewRuler, boolean showsAnnotationOverview,
			int styles, IPreferenceStore fPreferenceStore) {
	...
	private class AnnotationModelListener implements IAnnotationModelListener,
		...
		private void processModelChanged(IAnnotationModel model,
			...
			if (model == fCLRProjectionAnnotationModel) {
	...
	private CLRProjectionAnnotationModel fCLRProjectionAnnotationModel;
	...
	private CLRProjectionSummary fProjectionSummary;
	 ...
	 *            the projection summary.
	 */
	public void setProjectionSummary(CLRProjectionSummary projectionSummary) {
	...
	private void addProjectionAnnotationModel(IAnnotationModel model) {
		if (model instanceof IAnnotationModelExtension) {
			...
			extension.addAnnotationModel(
					CLRProjectionSupport.INLINEPROJECTION,
					fCLRProjectionAnnotationModel);
	...
	private IAnnotationModel removeProjectionAnnotationModel(
		...
		if (model instanceof IAnnotationModelExtension) {
			...
			IAnnotationModelExtension extension = (IAnnotationModelExtension) model;
			return extension
					.removeAnnotationModel(CLRProjectionSupport.INLINEPROJECTION);
		}
		return null;
	}
	...
	protected IAnnotationModel createVisualAnnotationModel(
		...
		fCLRProjectionAnnotationModel = new CLRProjectionAnnotationModel();
		return model;
	}
	 ...
	 */
	public CLRProjectionAnnotationModel getCLRProjectionAnnotationModel() {
		IAnnotationModel model = getVisualAnnotationModel();
		if (model instanceof IAnnotationModelExtension) {
			IAnnotationModelExtension extension = (IAnnotationModelExtension) model;
			return (CLRProjectionAnnotationModel) extension
					.getAnnotationModel(CLRProjectionSupport.INLINEPROJECTION);
		}
		return null;
	}
	 ...
	 */
	public final boolean isCLRProjectionMode() {
		return getCLRProjectionAnnotationModel() != null;
	}

	/**
	 * Disables the projection mode.
	 */
	public final void disableCLRProjection() {
		if (isCLRProjectionMode()) {
			removeProjectionAnnotationModel(getVisualAnnotationModel());
			fCLRProjectionAnnotationModel.removeAllAnnotations();
	 ...
	 */
	public final void enableCLRProjection() {
		if (!isCLRProjectionMode()) {
	...
	private void expandAll() {
		...
		if (isCLRProjectionMode()) {
			fCLRProjectionAnnotationModel.expandAll(offset, length);
		}
	}
	...
	private void expand() {
		if (isCLRProjectionMode()) {
			...
			for (Iterator e = fCLRProjectionAnnotationModel
					.getAnnotationIterator(); e.hasNext();) {
				CLRProjectionAnnotation annotation = (CLRProjectionAnnotation) e
						.next();
				if (annotation.isCollapsed()) {
					Position position = fCLRProjectionAnnotationModel
			...
			if (bestMatch != null) {
				fCLRProjectionAnnotationModel.expand(bestMatch);
	...
	private void collapse() {
		if (isCLRProjectionMode()) {
			...
			if (bestMatch != null) {
				fCLRProjectionAnnotationModel.collapse(bestMatch);
				revealRange(selection.x, selection.y);
			}
		}
	}
	...
	private void collapseAll() {
		...
		int length = doc == null ? 0 : doc.getLength();
		if (isCLRProjectionMode()) {
			fCLRProjectionAnnotationModel.collapseAll(offset, length);
		}
	}
...
//	}
	...
	public void setVisibleRegion(int start, int length) {
		if (!isSegmented())
			fWasInlineProjectionEnabled = isCLRProjectionMode();
		disableProjection();
	...
	private void expand(int offset, int length, boolean fireRedraw)
		...
		if (slave instanceof ProjectionDocument) {
			...
			CLRProjectionAnnotation[] collapsed = computeCollapsedNestedAnnotations(offset, length);
			if (collapsed != null) {
				for (int i = 0; i < collapsed.length; i++) {
					IRegion[] regions = computeCollapsedRegions(fCLRProjectionAnnotationModel
	...
	private CLRProjectionAnnotation[] computeCollapsedNestedAnnotations(
			int offset, int length) {
		List annotations = new ArrayList(5);
		Iterator e = fCLRProjectionAnnotationModel.getAnnotationIterator();
		while (e.hasNext()) {
		...
		if (annotations.size() > 0) {
			CLRProjectionAnnotation[] result = new CLRProjectionAnnotation[annotations.size()];
	...
	private void processDeletions(AnnotationModelEvent event,
		...
		for (int i = 0; i < removedAnnotations.length; i++) {
			CLRProjectionAnnotation annotation = (CLRProjectionAnnotation) removedAnnotations[i];
			if (annotation.isCollapsed()) {
	...
	private void processChanges(Annotation[] annotations, boolean fireRedraw,
		...
		for (int i = 0; i < annotations.length; i++) {
			CLRProjectionAnnotation annotation = (CLRProjectionAnnotation) annotations[i];
			Position position = fCLRProjectionAnnotationModel
					.getPosition(annotation);
...
//	}
			...
			try {
				...
				if (ProjectionDocumentEvent.PROJECTION_CHANGE == e
					...
					if (e.getLength() == 0 && replaceLength != 0)
						fCLRProjectionAnnotationModel.expandAll(e.getMasterOffset(), e.getMasterLength());
				} else if (master != null
					...
					try {
						...
						if (numberOfLines > 1 || fDeletedLines > 1)
							fCLRProjectionAnnotationModel.expandAll(master
	...
	public boolean canDoOperation(int operation) {
		...
		case EXPAND:
		case EXPAND_ALL:
			return isCLRProjectionMode();
		case TOGGLE:
			return isCLRProjectionMode() || !isSegmented();
		}
	...
	protected FindReplaceDocumentAdapter getFindReplaceDocumentAdapter() {
		if (fFindReplaceDocumentAdapter == null) {
			IDocument document = isCLRProjectionMode() ? getDocument()
					: getVisibleDocument();
	...
	private List fCLRProjectionListeners;
	...
	protected void fireInlineProjectionEnabled() {
		if (fCLRProjectionListeners != null) {
			Iterator e = new ArrayList(fCLRProjectionListeners).iterator();
			while (e.hasNext()) {
				ICLRProjectionListener l = (ICLRProjectionListener) e.next();
				l.CLRProjectionEnabled();
			}
		}
	}
	...
	public void addProjectionListener(IProjectionListener listener) {
		...
		if (listener instanceof ICLRProjectionListener) {

			if (fCLRProjectionListeners == null)
				fCLRProjectionListeners = new ArrayList();

			if (!fCLRProjectionListeners.contains(listener))
				fCLRProjectionListeners.add(listener);
		}
	}
	...
	public void removeProjectionListener(IProjectionListener listener) {
		...
		if (listener instanceof ICLRProjectionListener) {
			...
			if (fCLRProjectionListeners != null) {
				fCLRProjectionListeners.remove(listener);
				if (fCLRProjectionListeners.size() == 0)
					fCLRProjectionListeners = null;
			}
		}
	}
