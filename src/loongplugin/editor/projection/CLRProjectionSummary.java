...
public class CLRProjectionSummary {

	private CLRJavaSourceViewer fProjectionViewer;
	...
	public CLRProjectionSummary(CLRJavaSourceViewer projectionViewer, IAnnotationAccess annotationAccess){
		super();
	...
	private void createSummaries(IProgressMonitor monitor) {
		CLRProjectionAnnotationModel model= (CLRProjectionAnnotationModel) fProjectionViewer.getCLRProjectionAnnotationModel();
		if (model == null)
			return;
		...
		while (e.hasNext()) {
			CLRProjectionAnnotation projection= (CLRProjectionAnnotation) e.next();
