...
import loongplugin.typing.jdt.checks.FieldAccessCheck;
import loongplugin.typing.jdt.checks.InheritedMethodCheck;
import loongplugin.typing.jdt.checks.LocalVariableReferenceCheck;
import loongplugin.typing.jdt.checks.MethodInvocationCheck;
import loongplugin.typing.jdt.checks.TypeImportedCheck;
import loongplugin.typing.jdt.checks.TypeReferenceCheck;
	...
	protected void visitName(Name node) {
		...
		if (binding instanceof IVariableBinding
			...
			if (knownVariableDeclarations.get(binding) != null)
				checks.add(new LocalVariableReferenceCheck(file,
...
class JDTCheckGenerator_Imports extends JDTCheckGenerator_Methods {
	...
	public boolean visit(ImportDeclaration node) {
		...
		if (binding != null) {
			checks.add(new ImportTargetCheck(file, jdtTypingProvider,node, binding));
		}
		return super.visit(node);
	}
...
class JDTCheckGenerator_Methods extends JDTCheckGenerator_FieldAccess {
	...
	private void handleMethodCall(ASTNode node, IMethodBinding methodBinding,
		...
		checks.add(new MethodInvocationCheck(file, jdtTypingProvider,
				node, CheckUtils.getIASTNodeList(arguments),
				methodBinding));
	...
	private void handleOverridingRelationshipViolation(MethodDeclaration node,
		...
		checks.add(new InheritedMethodCheck(file, jdtTypingProvider,
				node, CheckUtils.getIASTNodeList(node.parameters()),
