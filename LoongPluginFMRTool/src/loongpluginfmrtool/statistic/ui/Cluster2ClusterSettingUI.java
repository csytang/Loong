...
import loongpluginfmrtool.statistic.c2c.Cluster2Cluster;

import org.eclipse.swt.widgets.Button;

public class Cluster2ClusterSettingUI extends TitleAreaDialog {
	 ...
	 * @param parentShell
	 */
	public Cluster2ClusterSettingUI(IProject pProject,Shell parentShell) {
		super(parentShell);
		aProject = pProject;
	...
	protected Control createDialogArea(Composite parent) {
		setTitle("Cluster2Cluster Setting Panel\n");
		...
		gd_lblValue.widthHint = 250;
		lblValue.setLayoutData(gd_lblValue);
		lblValue.setText("[max->min:MAX,MEDIUM,MIN]");
		new Label(container, SWT.NONE);
		new Label(container, SWT.NONE);
		...
		btnCompute.addListener(SWT.Selection, new Listener(){
			...
			public void handleEvent(Event event) {
				...
				if(combo.getSelectionIndex()!=-1){
					...
					Cluster2Cluster c2c = new Cluster2Cluster(recoveryResultTextContent.getText().trim(),groundTruthTextContent.getText().trim());
					String result = c2c.getResultStr();
					lblValue.setText(result);
				}
			}
