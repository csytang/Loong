...
package loongplugin.views.astview.views;
...
import loongplugin.views.astview.CleanASTColorAction;
import loongplugin.views.astview.EditorUtility;
import loongplugin.views.astview.IASTRefreshListener;
import loongplugin.views.astview.NodeFinder;
import loongplugin.views.astview.TreeInfoCollector;
...
public class ASTView extends ViewPart implements IShowInSource {
	...
	private IColoredJavaSourceFile fAnnotateSourceFile;
	...
	private static class ListenerMix implements ISelectionListener, IFileBufferListener, IDocumentListener,
	 ISelectionChangedListener, IDoubleClickListener, IPartListener2,IColorChangeListener,
		IASTRefreshListener {
		...
		@Override
		public void astRefreshed(CLRAnnotatedSourceFile sourceFile) {
			// TODO Auto-generated method stub
			if (sourceFile == fView.fAnnotateSourceFile)
		...
		public void astColorChanged(final ASTColorChangedEvent event) {
			// TODO Auto-generated method stub
			if (event.getColoredJavaSourceFile() == fView.fAnnotateSourceFile)
				Display.getDefault().asyncExec(new Runnable() {
					public void run() {
		...
		public void colorListChanged(ColorListChangedEvent event) {
			// TODO Auto-generated method stub
			if (fView.fAnnotateSourceFile == null)
				return;
			if (event.getProject() == fView.fAnnotateSourceFile.getProject()
	...
	public void init(IViewSite site) throws PartInitException {
		...
		if (fSuperListener == null) {
			...
			LoongPlugin.getDefault().addColorChangeListener(fSuperListener);
		}
	}
	...
	public void setInput(ITextEditor editor) throws CoreException {
		...
		if (editor != null) {
			...
			if (openable instanceof ICompilationUnit) {
				...
				fAnnotateSourceFile = CLRAnnotatedSourceFile.getColoredJavaSourceFile((ICompilationUnit) openable);
				ISelection selection = editor.getSelectionProvider()
						.getSelection();
				if (selection instanceof ITextSelection) {
					ITextSelection textSelection = (ITextSelection) selection;
					fRoot = internalSetInput(fAnnotateSourceFile,
	...
	private CompilationUnit internalSetInput(IColoredJavaSourceFile input, int offset, int length) throws CoreException {
	...
	protected void fillContextMenu(IMenuManager manager) {
		...
		manager.add(new Separator(IWorkbenchActionConstants.MB_ADDITIONS));
		List<ASTNode> nodes = new LinkedList<ASTNode>(getSelection());
		
		
		manager.add(new CleanASTColorAction(nodes, (CLRAnnotatedSourceFile) fAnnotateSourceFile));
	...
	private void makeActions() {
		...
		LoongImages.setImageDescriptors(fClearAction, LoongImages.CLEAR);
		...
		LoongImages.setImageDescriptors(fFocusAction, LoongImages.SETFOCUS);
		...
		LoongImages.setImageDescriptors(fCollapseAction, LoongImages.COLLAPSE);
		...
		LoongImages.setImageDescriptors(fExpandAction, LoongImages.EXPAND);
		...
		LoongImages.setImageDescriptors(fLinkWithEditor, LoongImages.LINK_WITH_EDITOR);
		...
		LoongImages.setImageDescriptors(fAddToTrayAction, LoongImages.ADD_TO_TRAY);
		...
		fDeleteAction.setEnabled(false);
		fDeleteAction.setImageDescriptor(LoongPlugin.getDefault().getWorkbench().getSharedImages().getImageDescriptor(ISharedImages.IMG_TOOL_DELETE));
	...
	private void refreshAST() throws CoreException {
		...
		fAnnotateSourceFile.refreshAST();
		internalSetInput(fAnnotateSourceFile, offset, length);
	}
