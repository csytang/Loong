...
//		return PlatformUI.getWorkbench().getSharedImages().getImage(imageKey);
	...
	public Color getBackground(Object element) {
		...
		return null;*/
		// this part has been updated to meet the color with color in editor
		ASTNode node = null;
		if (element instanceof ASTNode)
			node = (ASTNode) element;
		if (element instanceof NodeProperty
				&& ((NodeProperty) element).getParent() instanceof ASTNode)
			node = (ASTNode) ((NodeProperty) element).getParent();

		if (node != null) {
			Set<Feature> colors = fView.getColoredJavaSourceFile().getColorManager().getColors(node);
			if (colors.size() > 0) {
				RGB rgb = FeatureModelManager.getCombinedRGB(colors);
