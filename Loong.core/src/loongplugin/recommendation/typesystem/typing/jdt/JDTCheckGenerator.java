...
import loongplugin.recommendation.typesystem.typing.jdt.checks.FieldAccessCheck;
import loongplugin.recommendation.typesystem.typing.jdt.checks.FieldAssignmentCheck;
import loongplugin.recommendation.typesystem.typing.jdt.checks.ImportTargetCheck;
import loongplugin.recommendation.typesystem.typing.jdt.checks.InheritedMethodCheck;
import loongplugin.recommendation.typesystem.typing.jdt.checks.InheritedMethodExceptionCheck;
import loongplugin.recommendation.typesystem.typing.jdt.checks.LocalVariableReferenceCheck;
import loongplugin.recommendation.typesystem.typing.jdt.checks.MethodImplementationExceptionCheck;
import loongplugin.recommendation.typesystem.typing.jdt.checks.MethodImplementationNameCheck;
import loongplugin.recommendation.typesystem.typing.jdt.checks.MethodImplementationParameterCheck;
import loongplugin.recommendation.typesystem.typing.jdt.checks.MethodInvocationCheck;
import loongplugin.recommendation.typesystem.typing.jdt.checks.TypeImportedCheck;
import loongplugin.recommendation.typesystem.typing.jdt.checks.TypeReferenceCheck;
...
class JDTCheckGenerator extends JDTCheckGenerator_TypeReferences {
	...
	public JDTCheckGenerator(CLRAnnotatedSourceFile file,
...
class JDTCheckGenerator_TypeReferences extends JDTCheckGenerator_LocalVariables {
	...
	public JDTCheckGenerator_TypeReferences(CLRAnnotatedSourceFile file,
			JDTTypingProvider jdtTypingProvider, Set<ITypingCheck> checks) {
		super(file, jdtTypingProvider, checks);
	}
...
class JDTCheckGenerator_LocalVariables extends JDTCheckGenerator_Imports {
	...
	public JDTCheckGenerator_LocalVariables(CLRAnnotatedSourceFile file,
			JDTTypingProvider jdtTypingProvider, Set<ITypingCheck> checks) {
		super(file, jdtTypingProvider, checks);
	}

	private final HashMap<IVariableBinding, ASTNode> knownVariableDeclarations = new HashMap<IVariableBinding, ASTNode>();
	...
	private void visitVD(VariableDeclaration node) {
		...
		if (binding != null)
			knownVariableDeclarations.put(binding, node);
	}
...
class JDTCheckGenerator_Imports extends JDTCheckGenerator_Methods {
	...
	public JDTCheckGenerator_Imports(CLRAnnotatedSourceFile file,
			JDTTypingProvider jdtTypingProvider, Set<ITypingCheck> checks) {
		super(file, jdtTypingProvider, checks);
	}

	private final HashMap<ITypeBinding, ASTNode> importedTypes = new HashMap<ITypeBinding, ASTNode>();
	...
	public boolean visit(ImportDeclaration node) {
		...
		if (binding instanceof ITypeBinding) {
			importedTypes.put((ITypeBinding) binding, node);
		}
		if (binding != null) {
		...
		if (binding != null) {
			if (importedTypes.containsKey(binding)) {
				checks.add(new TypeImportedCheck(file, jdtTypingProvider,
						node, importedTypes.get(binding), node
								.toString()));
...
class JDTCheckGenerator_Methods extends JDTCheckGenerator_FieldAccess {
	public JDTCheckGenerator_Methods(CLRAnnotatedSourceFile file,
			JDTTypingProvider jdtTypingProvider, Set<ITypingCheck> checks) {
		super(file, jdtTypingProvider, checks);
	}
	...
	private boolean initializeAndCollectInhData(IMethodBinding binding,
		...
		OverridingRelationUtils.collectSimilarMethodKeysInSuperClasses(binding,
				declTypeBinding.getSuperclass(), inhMethods, checkedInterfaces);

		// (recursively) collects all keys of methods in interfaces which
		// belongs to this declaration
		OverridingRelationUtils.collectSimilarMethodKeysInInterfaces(binding,
	...
	private void handleInheritedAbstractMethodImplementation(
		...
		for (int j = 0; j < parameterList.size(); j++) {
			...
			checks.add(new MethodImplementationParameterCheck(file,
					jdtTypingProvider,
					(SingleVariableDeclaration) parameterList.get(j),
		...
		for (int i = 0; i < exceptionList.size(); i++) {
			...
			checks.add(new MethodImplementationExceptionCheck(file,
					jdtTypingProvider, curExcNode, methodBinding,
	...
	private void handleOverridingRelationshipViolation(MethodDeclaration node,
		...
		MethodPathItem superItem = OverridingRelationUtils
		...
		checks.add(new InheritedMethodCheck(file, jdtTypingProvider,
				node, OverridingRelationUtils.getIASTNodeList(node
		...
		for (int i = 0; i < exceptionList.size(); i++) {
			...
			checks.add(new InheritedMethodExceptionCheck(file,
					jdtTypingProvider, curExcNode, curExcBinding
...
class JDTCheckGenerator_FieldAccess extends JDTCheckGenerator_Base {
	public JDTCheckGenerator_FieldAccess(CLRAnnotatedSourceFile file,
			JDTTypingProvider jdtTypingProvider, Set<ITypingCheck> checks) {
		super(file, jdtTypingProvider, checks);
	}
	...
	private void handleFieldChecks(Name node, IVariableBinding binding) {
		...
		checks.add(new FieldAccessCheck(file, jdtTypingProvider, node,
				binding));
...
class JDTCheckGenerator_Base extends ASTVisitor {
	...
	public JDTCheckGenerator_Base(CLRAnnotatedSourceFile file,
			JDTTypingProvider jdtTypingProvider, Set<ITypingCheck> checks) {
		this.file = file;
