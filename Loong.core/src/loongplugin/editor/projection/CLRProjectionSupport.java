...
public class CLRProjectionSupport {
	 ...
	 * Painter 
	 * 包括 1. Annotation painter
	 * 2. AnnotationPainter.IDrawingStrategy
	 * 在这里我们这是 如何将高亮色彩加入到代码中 
	 * 更为重要的是 使用SourceViewer将加入的高亮 Annotation以实际的方式显示出来
	 */
	/**
	 * Key of the projection annotation model inside the visual annotation
	 * model. Also internally used as key for the projection drawing strategy.
	...
	private static class InlineProjectionAnnotationsPainter extends
		...
		protected IAnnotationModel findAnnotationModel(
			...
			if (sourceViewer instanceof CLRJavaSourceViewer) {
				CLRJavaSourceViewer projectionViewer = (CLRJavaSourceViewer) sourceViewer;
				return projectionViewer.getCLRProjectionAnnotationModel();
			}
			return null;
		}
		...
		protected boolean skip(Annotation annotation) {
			if (annotation instanceof CLRProjectionAnnotation)
				return !((CLRProjectionAnnotation) annotation).isCollapsed();
	...
	private static class InlineProjectionDrawingStrategy implements
		...
		public void draw(Annotation annotation, GC gc, StyledText textWidget,
			...
			if (annotation instanceof CLRProjectionAnnotation) {
				CLRProjectionAnnotation projectionAnnotation = (CLRProjectionAnnotation) annotation;
				if (projectionAnnotation.isCollapsed()) {
					...
					if (gc != null) {
						...
						if (annotation instanceof ColoredCLRProjectionAnnotation) {
							Set<Feature> colors = ((ColoredCLRProjectionAnnotation) annotation).getColors();
							Color combinedColor = FeatureModelManager.getCombinedColor(colors);
	...
	private class ProjectionListener implements IProjectionListener, ICLRProjectionListener {
		...
		public void CLRProjectionDisabled() {
			doInlineDisableProjection();
		}

		public void CLRProjectionEnabled() {
			doInlineEnableProjection();
		}
	}

	private CLRJavaSourceViewer fViewer;
	...
	private CLRProjectionRulerColumn fColumn;
	 ...
	 */
	public CLRProjectionSupport(CLRJavaSourceViewer viewer,
	...
	protected void doInlineEnableProjection() {
		...
		if (fPainter == null) {
			...
			fPainter.addAnnotationType(CLRProjectionAnnotation.TYPE,
					INLINEPROJECTION);
			fPainter.setAnnotationTypeColor(CLRProjectionAnnotation.TYPE,
					fSharedTextColors.getColor(getColor()));
			fViewer.addPainter(fPainter);
		}

		if (fColumn == null) {
			fColumn = new CLRProjectionRulerColumn(9, fAnnotationAccess);
			fColumn.addAnnotationType(CLRProjectionAnnotation.TYPE);
...
//	}
	...
	private IAnnotationHover createProjectionAnnotationHover() {
		CLRProjectionAnnotationHover hover = new CLRProjectionAnnotationHover();
	...
	public Object getAdapter(ISourceViewer viewer, Class required) {
		if (CLRProjectionAnnotationModel.class.equals(required)) {
			if (viewer instanceof CLRJavaSourceViewer) {
				CLRJavaSourceViewer projectionViewer = (CLRJavaSourceViewer) viewer;
				return projectionViewer.getCLRProjectionAnnotationModel();
			}
		}
		return null;
