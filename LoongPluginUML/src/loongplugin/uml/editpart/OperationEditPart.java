...
import loongplugin.uml.classdiagram.figure.OperationLabel;
...
public class OperationEditPart extends AbstractUMLEditPart {
		...
		if (model.isShowIcon()) {
			if (model.getVisibility().equals(Visibility.PUBLIC)) {
				if(model.isConstructor()){
					label.setIcon(LoongUMLPlugin.getImageDescriptor("icons/const_public.gif").createImage());
				} else {
					label.setIcon(LoongUMLPlugin.getImageDescriptor("icons/method_public.gif").createImage());
				}
			} else if (model.getVisibility().equals(Visibility.PRIVATE)) {
				if(model.isConstructor()){
					label.setIcon(LoongUMLPlugin.getImageDescriptor("icons/const_private.gif").createImage());
				} else {
					label.setIcon(LoongUMLPlugin.getImageDescriptor("icons/method_private.gif").createImage());
				}
			} else if (model.getVisibility().equals(Visibility.PROTECTED)) {
				if(model.isConstructor()){
					label.setIcon(LoongUMLPlugin.getImageDescriptor("icons/const_protected.gif").createImage());
				} else {
					label.setIcon(LoongUMLPlugin.getImageDescriptor("icons/method_protected.gif").createImage());
				}
			} else if (model.getVisibility().equals(Visibility.PACKAGE)) {
				if(model.isConstructor()){
					label.setIcon(LoongUMLPlugin.getImageDescriptor("icons/const_default.gif").createImage());
				} else {
					label.setIcon(LoongUMLPlugin.getImageDescriptor("icons/method_default.gif").createImage());
				}
			}
	...
	private boolean showParameterName() {
		return LoongUMLPlugin.getDefault().getPreferenceStore().getBoolean(LoongUMLPlugin.PREF_CLASS_DIAGRAM_SHOW_PARAMETER_NAME);
	}
}
